...
public class CartController {
    ...
    public Result<CartItemVO> add(@RequestBody CartAddRequest request) {
    ...
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "成功",
                    content = @Content(examples = @ExampleObject(value = "{\"code\":200,\"message\":\"成功\",\"data\":{\"id\":10,\"productId\":1,\"productName\":\"茉莉绿茶\",\"productPrice\":12.00,\"quantity\":3,\"imageUrl\":\"https://example.com/p1.jpg\"}}"))
            ),
            @ApiResponse(responseCode = "400", description = "参数错误或库存不足"),
    ...
    public Result<CartItemVO> update(@RequestBody CartUpdateRequest request) {
        Long userId = currentUserId();
        return Result.success(cartService.updateCart(userId, request.getId(), request.getQuantity()));
    }
